...
#define next_4bytes_boundary(x) \
		(typeof(x)) ((((uint64_t)x) + 3) & ~0x03)
							 ...
							 uint64_t *offset)
{
	...
	stap_note_section_data = lttng_elf_get_section_data(elf, strtab_hdr);
	...
	for (sym_idx = 0; sym_idx < sym_count; sym_idx++) {
		 ...
		 */
		curr_sym_str = stap_note_section_data + curr_sym.st_name;
	...
	if (ret) {
		ERR("Cannot convert address to offset.");
		goto end;
	}
							 ...
							 uint64_t *addr)
{
	...
	char *stap_note_section_data = NULL;
	struct lttng_elf *elf;
	struct lttng_elf_shdr *note_section_hdr = NULL;

	if (!provider || !probe_name || !addr ) {
		ret = -1;
		goto end;
	}
	elf = lttng_elf_create(fd);
	...
	stap_note_section_data = lttng_elf_get_section_data(elf, note_section_hdr);
	if (stap_note_section_data == NULL) {
	...
	curr_note_beg = stap_note_section_data;
	while (1) {
		/* Have we reached the end of the note section ? */
		if (curr_data_ptr >= note_section_hdr->sh_size + curr_note_beg) {
			ret = -1;
			break;
		}
		/* Get namesz field*/
		int namesz =*(uint32_t*)curr_data_ptr;
		//bswap(namesz);
		...
		/* Get descsz field*/
		int descsz = *(uint32_t*) curr_data_ptr;
		curr_data_ptr += sizeof(uint32_t);


		/* Get type field*/
		int type = *(uint32_t *) curr_data_ptr;
		curr_data_ptr += sizeof(uint32_t);

		assert(type == 3);

		/* Move ptr to the end of the name string and go to the next 4 byte
		 * alignement */
		curr_data_ptr += namesz;
		curr_data_ptr = next_4bytes_boundary(curr_data_ptr);

		/* Get desc field*/
		char *curr_desc_beg = curr_data_ptr;

		/* Get location */
		uint64_t location = *(uint64_t *) curr_data_ptr;
		curr_data_ptr += sizeof(uint64_t);

		/* Get base */
		uint64_t base = *(uint64_t *) curr_data_ptr;
		curr_data_ptr += sizeof(uint64_t);

		/* Get semaphore */
		uint64_t semaphore = *(uint64_t *) curr_data_ptr;
		curr_data_ptr += sizeof(uint64_t);

		/* Get provider */
		char *prov = curr_data_ptr;
		curr_data_ptr += strlen(prov) + 1;

		/* Get probename */
		char *prob = curr_data_ptr;
		curr_data_ptr += strlen(prob) + 1;

		curr_data_ptr = curr_desc_beg + descsz;
		curr_data_ptr = next_4bytes_boundary(curr_data_ptr);

		printf("type: %d, location: 0x%lx, base: 0x%lx, sema: 0x%lx, prov:%s, prov:%s\n",type, location, base, semaphore, prov, prob);
		if (strcmp(provider, prov) == 0 && strcmp(probe_name, prob) == 0) {
			*addr = semaphore;
			ret = 0;
			break;
		}
